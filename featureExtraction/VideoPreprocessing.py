...
def get_frames(my_segment, size=30):
    ...
    frames = np.array(frames).reshape((3, common['FRAME_SIZE'], common['FRAME_SIZE'], -1))
    try:
...
def split_video(video_path, num_segments, num_frames):
    ...
    bagof_segments = np.zeros((num_segments, 3, num_frames, common['FRAME_SIZE'], common['FRAME_SIZE']))
    # Load the video clip
    clip = VideoFileClip(video_path)
    clip = clip.resize((common['FRAME_SIZE'], common['FRAME_SIZE']))
